module.exports = function (grunt) {
	grunt.initConfig({
		...
		git_deploy: {
			app: {
				options: {
					url: 'https://github.com/Minkov/grunt-deploy-demo.git',
					branch: 'dev'
				},
				src: '<%= project.app %>'
			},
	...
	grunt.registerTask('build', ['jshint', 'stylus', 'csslint', 'clean:build', 'concat', 'uglify', 'cssmin', 'copy', 'processhtml']);
	grunt.registerTask('serve', ['jshint', 'stylus', 'csslint', 'connect', 'watch']);
	grunt.registerTask('deploy', function () {
		grunt.task.run('build', 'git_deploy:build');
		grunt.task.run('build', 'git_deploy:app');
	});
